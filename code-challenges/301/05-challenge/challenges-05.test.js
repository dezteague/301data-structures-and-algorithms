...
const howMuchPencil = (str) => {
  let result = [];
  for (let i = 0; i < str.length; i++) {//runs through the string
    let newString = str.slice(i, str.length);//slices first letter of index values
    result.push(newString);// pushes new string into results array
  }
  result.push('');//this stringifies the results array
  return result;
};
...
const wordsToCharList = (arr) => {
  let words = arr.split(''); //takes in the elements and make an array, then split element
  return words;
};
...
const listFoods = (recipe) => {
  ...
  let ingOne=recipe.ingredients[0].slice(15);
  result.push(ingOne);
  let ingTwo=recipe.ingredients[1].slice(9);
  result.push(ingTwo);
  let ingThree=recipe.ingredients[2].slice(9);
  result.push(ingThree);
  let ingFour=recipe.ingredients[3].slice(9);
  result.push(ingFour);
  let ingFive=recipe.ingredients[4].slice(10);
  result.push(ingFive);
  let ingSix=recipe.ingredients[5].slice(8);
  result.push(ingSix);
  let ingSeven=recipe.ingredients[6].slice(8);
  result.push(ingSeven);
  let ingEight=recipe.ingredients[7].slice(8);
  result.push(ingEight);
  let ingNine=recipe.ingredients[8].slice(8);
  result.push(ingNine);
  let ingTen=recipe.ingredients[9].slice(10);
  result.push(ingTen);
  let ingEleven=recipe.ingredients[10].slice(10);
  result.push(ingEleven);
